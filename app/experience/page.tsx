        ...
        <main id='main-scroller' className='h-screen overflow-y-auto bg-foreground text-background'>
            <section className='relative mx-auto w-full max-w-6xl px-6 pb-20 pt-40 md:px-10'>
                <div className='relative mb-10 flex flex-wrap items-end justify-between gap-4'>
                    <div>
                        <p className='mb-2 text-sm font-semibold uppercase tracking-[0.2em] text-primary font-fira-code'>Career Path</p>
                    ...
                    {experiences.map((experience) => (
                        <article key={`${experience.company}-${experience.period}`} className='rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-md'>
                            <div className='mb-4 flex flex-wrap items-center justify-between gap-3'>
                                <span className='rounded-full border border-primary/40 bg-primary/20 px-3 py-1 text-xs text-primary'>
                                    {experience.period}
                                </span>
                            ...
                            <ul className='mb-5 space-y-2 text-background'>
                            ...
                            <div className='flex flex-wrap gap-2'>
                                {experience.stack.map((tech) => (
                                    <span key={`${experience.role}-${tech}`} className='rounded-full border border-white/15 bg-black/30 px-3 py-1 text-xs text-background'>
                                        {tech}
                                    </span>
                                ))}
                            </div>
